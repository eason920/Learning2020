...
$(function () {
	...
	let landHeight = $('.landbtn').height();



	// FB IN IPHONE X BTN BUG
	// console.log(nua);
	
	const fnua = navigator.userAgent || navigator.vendor || window.opera;
	const ww = $(window).width();
	const wh = $(window).height();
	if(nua.indexOf('iPhone') > 0){
		if( ww > 360 && wh > 600 ){
			if( fnua.indexOf("FBAN") > -1 || fnua.indexOf("FBAV") > -1 ){
				$('.main').addClass('is-fb')
			}else{
				// console.log('not fb');
			}		
		}else{
			// console.log('width not small then 360');
		}	
	}else{
	...
	if( layoutType == 'a' ){
		...
		$('.Read_More_btn').click(function () {
			...
			// const adHeight = $('.landbtn').height() - 1; // -1 for strange gray vision above the btn  // landbtn height
			...
			const scrollHeight = 'calc( 100% - ' + height + 'px - ' + landHeight + 'px )';
	...
	if( layoutType == 'b'){
		...
		const wh = $(window).height();
		const uh = function(){
			$('.article_cover').removeAttr('style');
			setTimeout(function(){
				const height =	$('.article_cover').height();
				$('.article_cover').css({height});
				$('.scroller').css({height: wh - height - landHeight}); // << landbtn height = 100			
			},300);
		};
